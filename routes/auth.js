...
router.post('/register', validate('register'), async (req, res) => {
    try {
        ...
        const existingUser = await database.get('SELECT * FROM users WHERE email = ? OR username = ?', [email, username]);
        if (existingUser) {
...
router.post('/login', validate('login'), async (req, res) => {
    try {
        ...
        const userData = await database.get('SELECT * FROM users WHERE email = ? OR username = ?', [identifier, identifier]);
        if (!userData) {
